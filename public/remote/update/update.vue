            ...
            <div class="update-desc-container" v-else>
                <div class="update-desc-title">
                    <div class="update-desc-version">
                        <span class="beta" v-if="updated?.status === '10'">（内测版）</span>
                        有版本更新 {{ updated.version_name }}（{{ updated.version }}）
                    </div>
...
export default {
    ...
    methods: {
        ...
        hanldStepUpdate(step) {
            ...
            // 发送更新请求
            const params = {
                step: this.stepData.step,
                version: this.updated.version,
                client_version: this.updated.client_version,
            }
            _this.$http.usePost('admin/Update/update', params).then((res) => {
                if (res?.data?.next === '') {
        ...
        getDetail() {
            this.$http.useGet("admin/Update/getUpdate").then((res) => {
                ...
                // 没有版本更新
                this.$emit('update:openWin', 'remote/update/index')
            })
        },
    },
}
