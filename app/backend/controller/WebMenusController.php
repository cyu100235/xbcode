...
use app\validate\WebMenusValidate;
 ...
 */
class WebMenusController extends XbController
{
    // 引入可编辑行
    use RowEditTrait;
    ...
    public function indexTable(Request $request)
    {
        ...
        $description = <<<STR
        <div>非开发者，请勿直接在此添加菜单，该菜单仅用于插件自动操作菜单数据或特殊需求时添加</div>
        STR;
        ...
        $builder->addTopButton('add', '添加站点菜单', [
            'type' => 'modal',
            'api' => xbUrl('WebMenus/add'),
            'path' => xbUrl('WebMenus/add'),
        ], [
            'title' => '添加站点菜单',
            'customStyle' => [
                'width' => '500px',
        ...
        $builder->addRightButton('edit', '资源', [
            'type' => 'modal',
            'api' => xbUrl('WebMenus/resources'),
            'path' => xbUrl('WebMenus/resources'),
        ], [
            'title' => '站点资源菜单',
            'customStyle' => [
                'width' => '500px',
        ...
        $builder->addRightButton('edit', '修改', [
            'type' => 'modal',
            'api' => xbUrl('WebMenus/edit'),
            'path' => xbUrl('WebMenus/edit'),
        ], [
            'title' => '修改菜单',
            'customStyle' => [
                'width' => '500px',
        ...
        $builder->addRightButton('del', '删除', [
            'type' => 'confirm',
            'api' => xbUrl('WebMenus/del'),
            'path' => xbUrl('WebMenus/del'),
            'method' => 'delete',
        ], [
            'type' => 'error',
            'title' => '温馨提示',
            'content' => '站点子菜单也会将其删除，是否确认删除该数据？',
        ], [
            'type' => 'danger',
            'icon' => 'Close',
        ]);
    ...
    public function add(Request $request)
    {
        if ($request->method() === 'POST') {
            ...
            $post = $request->post();
            // 数据验证
            xbValidate(WebMenusValidate::class, $post, 'add');
            // 获取父级ID
    ...
    public function edit(Request $request)
    {
        ...
        if ($request->method() === 'PUT') {
            ...
            $post = $request->post();
            // 数据验证
            xbValidate(WebMenusValidate::class, $post, 'edit');
            // 获取父级ID
            if (is_array($post['pid'])) {
                $post['pid'] = end($post['pid']);
            }
