 ...
 */
class PluginsUpdateRollbackProvider
{
    /**
     * 插件名称
     * @var string
     ...
     * @param bool $database 是否执行数据回滚
     * @return void
     * @copyright 贵州小白基地网络科技有限公司
     * @author 楚羽幽 cy958416459@qq.com
     */
    public function start(string $name, string $versionName, int $version, bool $database = false)
    {
        // 插件名称
        ...
        try {
            ...
            throw new Exception("插件代码回滚出错：{$th->getMessage()}");
        }
        try {
            // 执行数据回滚
            if ($database) {
            ...
            throw new Exception("插件数据库回滚出错：{$th->getMessage()}");
        }
    }
    ...
    private function rollbackCode()
    {
        ...
        // 检测备份文件是否存在
        if (!file_exists($this->backupPluginCodePath)) {
            throw new Exception("代码备份文件不存在");
        }
        // 删除插件目录
        ...
        // 解压插件备份文件至插件目录
        ZipUtil::unzip($this->backupPluginCodePath, $this->pluginPath);
    }
    ...
    {
        // 检测备份文件是否存在
        if (!file_exists($this->backupPluginSqlPath)) {
            throw new Exception("数据备份文件不存在");
        }
        // 当前表前缀
        ...
        // 导入备份数据
        MysqlUtil::importSql($this->backupPluginSqlPath, '__PREFIX__', $prefix);
    }
}