 ...
 */
class PclZipUtil implements ZipInterface
{
    /**
     * 构造函数
     * @copyright 贵州小白基地网络科技有限公司
    ...
    {
        $zip        = new PclZip($zipFilePath);
        // $zip->create('file.txt,data/text.txt,folder');
        // 执行递归打包
        foreach ($files as $file) {
            $filePath = "{$extractTo}/{$file}";
            if (is_file($filePath) && file_exists($filePath)) {
                $zip->add($filePath, $file);
     ...
     * @copyright 贵州小白基地网络科技有限公司
     */
    private function addFileToZip(PclZip $zip, string $sourcePath, string $zipPath = '/', array $ignoreFiles = [])
    {
