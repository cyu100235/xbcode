...
class XbMiddleware
{
    ...
    private function initPlugin()
    {
        ...
        $appDir = 'base';
        // 应用ID
        $request->saasAppid = $project->id;
        // 项目名称
        $request->appTitle = $project->title;
        // 项目图标
        $request->appLogo = $project->logo;
        // 设置应用名称
    ...
    protected static function getAppInfo(string $projectName)
    {
        ...
        $expire = 60;
        // 查询项目数据
        $project = Projects::where('name', $projectName)
        ->cache($projectName,$expire)
        ->find();
        if (!$project) {
