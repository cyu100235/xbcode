...
abstract class PluginsBaseApi
{
    ...
    protected function setPluginInfo(string $name, string $version)
    {
        ...
        if (!is_dir($this->pakcageDir)) {
            mkdir($this->pakcageDir, 0777, true);
        }
    ...
    protected function upload()
    {
        ...
        if ($extension !== 'zip') {
            throw new Exception('请上传zip格式的插件包文件');
        }
        // 获取文件路径
        ...
        // 检测插件是否已经安装
        if (PluginsApi::make()->installed($plugin['name'])) {
    ...
    protected function unzip()
    {
        ...
        try {
            ...
            // 检查解压目录结构
            if (!file_exists("{$this->pluginPath}/plugins.json")) {
                $data = DirUtil::tree($this->pluginPath);
                if (count($data) > 1) {
                    throw new Exception('插件包解压目录结构错误');
                }
                $data = current($data);
                if (empty($data['path'])) {
                    throw new Exception('获取解压后临时插件目录失败');
                }
                DirUtil::copyDir($data['path'], $this->pluginPath);
                // 删除目录
                if (is_dir($data['path'])) {
                    DirUtil::delDir($data['path']);
                }
            }
        } catch (\Throwable $th) {
            // 检测插件包是否存在
    ...
    protected function script()
    {
        ...
        if (!class_exists($class)) {
            throw new Exception('插件安装脚本类不存在');
        }
        // 实例安装累
        $class = new $class;
        // 执行前置方法
