...
class MenuChecked
{
     ...
     * @author 楚羽幽 958416459@qq.com
     */
    public static function menusNo(array $menus, int $pid = 0, int $id = 1)
    {
        foreach ($menus as $key => $value) {
            $menus[$key]['pid'] = $pid;
            $menus[$key]['id'] = $id;
            if (!empty($value['children'])) {
                $menus[$key]['children'] = self::menusNo($value['children'], $id, $id + 1);
                $id = end($menus[$key]['children'])['id'];
            }
            $id++;
        }
        return $menus;
    ...
    protected static function parseMenuData(array $data)
    {
        ...
        // 判断是否工作台菜单
        if ($data['path'] === 'workbench') {
            $url = $data['params']['url'] ?? '';
            $data['params'] = "app/{$data['plugin']}/{$url}";
        } else if (!str_contains($data['path'], 'workbench') && str_contains($data['path'], '/')) {
            // 替换路径
