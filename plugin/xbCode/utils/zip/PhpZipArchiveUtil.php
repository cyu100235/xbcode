...
class PhpZipArchiveUtil implements ZipInterface
{
    ...
    public function build(string $zipFilePath, string $extractTo, array $ignoreFiles = [])
    {
        ...
        if (!is_writable($extractTo)) {
            throw new Exception("打包目录无权限：{$extractTo}");
        }
        $zip = new ZipArchive;
        ...
        // 执行递归打包（zipPath 传空字符串，避免路径以 / 开头导致解压出现 _ 目录）
    ...
    public function buildFiles(string $zipFilePath, string $extractTo, array $files)
    {
        // 检测目标目录是否有权限
        if (!is_writable($extractTo)) {
            throw new Exception("解压目录无权限：{$extractTo}");
        }
        $zip = new ZipArchive;
        $openStatus = $zip->open($zipFilePath, ZipArchive::CREATE);
        if ($openStatus !== true) {
    ...
    private function addFileToZip(ZipArchive $zip, string $extractTo, string $zipPath = '', array $ignoreFiles = [], $local_parent_path = null)
    {
        ...
        foreach ($files as $file) {
            if ($file != "." && $file != "..") {
                ...
                // 拼接 zip 内路径，避免以 / 开头（ZipArchive 会将开头的 / 转成 _）
                $entryPath = $zipPath !== '' ? $zipPath . '/' . $file : $file;
                if (is_dir($path)) {
                    if (!in_array(rtrim($local_path, '/'), $ignoreFiles)) {
                        $zip->addEmptyDir($entryPath);
                        $this->addFileToZip($zip, $path, $entryPath, $ignoreFiles, $local_path);
                    }
