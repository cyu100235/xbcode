...
final class DataUtil
{
     ...
     * @return array
     */
    public static function channelLevel($data, $pid = 0, $html = "&nbsp;", $fieldPri = 'cid', $fieldPid = 'pid', $level = 1, $children = 'children')
    {
        if (empty($data)) {
            return [];
        }
     ...
     * @author 楚羽幽 958416459@qq.com
     */
    public static function channelTree(array $data, int $pid = 0, string $idField = 'id', string $pidField = 'pid', string $children = 'children')
    {
        // 获取数据
        $data = self::channelLevel($data, $pid, '', $idField, $pidField, 1, $children);
        // 重置数组多层级下标
        $data = self::_channelTree($data);
        return $data;
    }
     ...
     * @author 楚羽幽 958416459@qq.com
     */
    public static function channelOptions(array $data, string $labelField = 'id', string $valueField = 'title')
    {
        $data = array_map(function ($value) use ($labelField, $valueField) {
            return [
                'label' => $value[$labelField],
                'value' => $value[$valueField],
                'children' => static::channelOptions($value['children'], $labelField, $valueField)
            ];
        }, $data);
        return $data;
    }
     ...
     * @author 楚羽幽 958416459@qq.com
     */
    private static function _channelTree(array $data)
    {
        $list = [];
        foreach ($data as $value) {
            ...
            if (!empty($value['children'])) {
                $value['children'] = static::_channelTree($value['children']);
            }
            $list[] = $value;
        }
        return $list;
     ...
     * @author 楚羽幽 958416459@qq.com
     */
    public static function restIndexList(array $data)
    {
        $list = [];
        foreach ($data as $value) {
            if (!empty($value['children'])) {
                $value['children'] = static::restIndexList($value['children']);
            }
            $list[] = $value;
        }
        return $list;
     ...
     * @param string $fieldPid 父id
     * @param string $ico 图标名称
     * @param string $more_class 是否添加更多类名 默认：NULL
     * @param string $class1 子级分类1图标
     * @param string $class2 子级分类结尾图标
     * @return array
     */
    static public function tree($data, $title, $fieldPri = 'cid', $fieldPid = 'pid', $ico = 'icon-bars', $more_class = NULL, $class1 = '├ ', $class2 = '└ ')
    {
        ...
        foreach ($arr as $k => $v) {
            ...
            if ($v['_level'] != 1) {
                ...
                if (isset($arr[$k + 1]) && $arr[$k + 1]['_level'] >= $arr[$k]['_level']) {
                    $arr[$k]['_name'] = $str . $class1 . $v['_html'] . $t;
                } else {
                    $arr[$k]['_name'] = $str . $class2 . $v['_html'] . $t;
                }
            } else {
                $arr[$k]['_name'] = "<span class='" . $ico . " " . $more_class . "'> " . $v[$title] . "</span>";
            }
        }
        //设置主键为$fieldPri
        $data = array();
