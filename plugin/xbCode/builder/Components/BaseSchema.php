...
abstract class BaseSchema implements JsonSerializable
{
    ...
    public function getVariables()
    {
        if (method_exists($this::class, 'defaultAttr')) {
            // 记录调用 defaultAttr 前已设置的属性，避免默认值覆盖手动设置的值
            ...
            // 将 defaultAttr 新增的属性（调用前不存在）合并进来
    ...
    public function jsonSerialize(): array
    {
        ...
        // 如果组件有 visibleOn 且有 body 数组，同步 visibleOn 给 body 中的子组件
        if (isset($vars['visibleOn']) && isset($vars['body']) && is_array($vars['body'])) {
            foreach ($vars['body'] as &$item) {
                if ($item instanceof BaseSchema && !isset($item->visibleOn)) {
                    $item->visibleOn($vars['visibleOn']);
                    unset($vars['visibleOn']);
                }
            }
        }
        return $vars;
