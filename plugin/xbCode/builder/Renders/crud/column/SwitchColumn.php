...
trait SwitchColumn
{
     ...
     * @author 楚羽幽 958416459@qq.com
     */
    public function addColumnSwitch(string $name, string $label, array $quickEdit = [])
    {
        if (empty($this->useCRUD()->quickSaveItemApi)) {
            throw new \Exception('请先设置【quickSaveItemApi】接口地址');
        }
        /** @var TableColumn|AmisSwitch */
        $component = $this->useCustomColumn(AmisSwitch::class, $name, $label);
        $component->width(120);
     ...
     * - `mode` 显示模式，默认为`inline`
     * @throws \Exception
     * @return AmisSwitch|TableColumn
     * @copyright 贵州积木云网络科技有限公司
     * @author 楚羽幽 958416459@qq.com
     */
    public function addColumnSwitchApi(string $name, string $label, string $api, array $quickEdit = [])
    {
        /** @var TableColumn|AmisSwitch */
        $component = $this->useCustomColumn(AmisSwitch::class, $name, $label);
        $component->width(120);
