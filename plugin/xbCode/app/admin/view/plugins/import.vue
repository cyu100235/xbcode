...
export default {
            ...
            uploadProps: {
                ...
                onSuccess: (res) => {
                    if (res?.status === 0 && res?.data?.plugin) {
                        ...
                        this.addStep(res?.data?.text ?? '插件上传成功，开始解压...', 'upload', 'success')
                        setTimeout(() => {
    ...
    methods: {
        hanldeImport() {
            ...
            const url = 'app/xbCode/admin/Plugins/import'
            const data = {
                step: this.progress.step,
                name: this.progress.plugin.name,
                version: this.progress.plugin.version,
            }
            this.$xbcode.$http.post(url, data).then(res => {
                if (step.next === 'success') {
                    this.addStep(res?.data?.text ?? '插件导入完成...', 'success', 'success');
                    setTimeout(() => {
                ...
                } else if (step.next && res?.data?.text) {
                    ...
                    this.addStep(res?.msg ?? '插件导入失败', 'fail', 'error')
                }
            }).catch(err => {
                this.addStep(err?.response?.data?.msg ?? err?.message, 'fail', 'error')
            })
        },
