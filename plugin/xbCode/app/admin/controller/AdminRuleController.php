...
use plugin\xbCode\enum\ComponentEnum;
use plugin\xbCode\enum\MenuStyleEnum;
use plugin\xbCode\app\model\AdminRule;
use plugin\xbCode\builder\FormBuilder;
use plugin\xbCode\builder\ListBuilder;
use plugin\xbCode\enum\ComponentStyleEnum;
...
class AdminRuleController extends XbController
{
    ...
    public function indexTable(Request $request)
    {
        ...
        $builder->addColumn('type', '菜单类型', [
            ...
            'params' => [
                'type' => 'dict',
                'props' => [
                    'options' => MenuTypeEnum::dict(),
                    'style' => MenuStyleEnum::style(),
                ],
            ],
        ]);
        ...
        $builder->addColumn('component', '组件类型', [
            ...
            'params' => [
                'type' => 'dict',
                'props' => [
                    'options' => ComponentEnum::dict(),
                    'style' => ComponentStyleEnum::style(),
                ],
            ],
        ]);
    ...
    public function del(Request $request)
    {
        ...
        // 递归查询子菜单ID
        $ids = MenuData::getChildrenIds($id);
        // 批量删除
    ...
    public function resources(Request $request)
    {
        ...
        if ($request->method() === 'POST') {
            ...
            $menuData = $model->toArray();
            // 创建资源菜单
            MenuResponse::addResponse($menuData, $post);
            // 返回数据
            return $this->success('生成资源菜单成功');
        }
        // 获取资源菜单选项
        $options = MenuResponse::responseOption();
        foreach ($options as &$item) {
            // 检测是否表格
            if ($model['component'] != 'table/index') {
                $item['disabled'] = true;
                continue;
            }
