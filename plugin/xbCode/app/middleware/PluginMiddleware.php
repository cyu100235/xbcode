...
class PluginMiddleware implements MiddlewareInterface
{
    ...
    protected function pluginValidate(Request $request)
    {
        ...
        // 检测插件是否存在
        if (!PluginsApi::make()->exists($pluginNmae)) {
            throw new ExceptionInternalServerError("插件 {$pluginNmae} 不存在");
        }
        // 检测是否已安装
        if (!PluginsApi::make()->installed($pluginNmae)) {
            throw new ExceptionInternalServerError("该插件 {$pluginNmae} 未安装");
        }
        // 检测插件是否已启用
        if (!PluginsApi::make()->hasEnabled($pluginNmae)) {
            throw new ExceptionInternalServerError("该插件 {$pluginNmae} 未启用");
        }
    }
}