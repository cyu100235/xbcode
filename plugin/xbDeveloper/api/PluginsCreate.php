...
class PluginsCreate
{
    ...
    private static $dirRemarks = [
        ...
        'app/home/view' => '默认视图目录',
        'app/home/view/index' => '默认首页视图目录',
        'app/model' => '模型目录',
        'config' => '配置目录',
        'enum' => '枚举目录',
        'data' => '数据目录',
    ...
    public static function create(array $data, bool $output = false)
    {
        ...
        try {
            ...
            // 未连接数据库时创建则不抛出错误
            if (str_contains($th->getMessage(), 'Access denied for user')) {
                return;
            }
            throw $th;
        }
    ...
    public static function clone(array $data, bool $output = false)
    {
        ...
        // 克隆仓库
        GitUtil::subModules($data['url'], $data['name']);
        // 是否输出日志
        static::$output = $output;
        // 插件信息
        static::$plugin = $data;
    ...
    public static function validate(array $data)
    {
        ...
        if ($titleCount < 2 || $titleCount > 20) {
            throw new Exception('插件名称长度为2-20个字');
        }
    ...
    private static function mkPluginRemarks(string $pluginPath)
    {
        ...
        file_put_contents("{$pluginPath}/remarks.txt", "Create by 积木云\n\n{$content}");
    }
    ...
    private static function mkdir(string $path, string $content, string $name)
    {
        ...
        file_put_contents("{$path}/remarks.txt", "Create by 积木云\n\n{$content}");
    }
