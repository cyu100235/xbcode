...
export default {
    ...
    methods: {
        hanldExport(name) {
            ...
            const url = '/app/xbDeveloper/admin/Index/export'
            const data = {
                active: name,
                name: this.plugin.name,
            }
            const plugin = this.plugin
            this.$xbcode.usePost(url, data).then(res => {
                if (res.status === 0) {
                    this.addStep(`导出 ${activeData.title} 数据成功`, name, 'success')
                    this.addStep('', name)
                } else {
                    this.addStep(`导出 ${activeData.title} 数据失败`, name, 'error')
                }
            }).catch((err) => {
                const error = err?.message || `导出 ${activeData.title} 数据失败`
                this.addStep(error, name, 'error')
            }).finally(() => {
                ...
                if (activeIndex > -1) {
                    ...
                    if (nextName) {
                        ...
                        setTimeout(() => {
                            this.addStep(`正在导出 ${next.title}数据...`, 'detail', 1)
                            this.hanldExport(next?.name);
                        }, 1500);
                    } else {
                        this.addStep('插件数据导出完成...', 'success', 'success', 1)
                        this.addStep('窗口将于3秒后，自动关闭...', 'success', 'success', 1)
                        setTimeout(() => {
                            this.$emit('close')
                        }, 3000);
                    }
        ...
        hanldSubmit() {
            if (!this.checkedTypes.active.length) {
                this.$xbcode.useNotify('请至少选择一个导出类型', 'error')
                return;
            }
            if (!this.plugin) {
                this.$xbcode.useNotify('插件数据参数错误', 'error')
                return;
            }
            this.$xbcode.useConfirm('是否确定导出选择的数据类型？', 'warning').then(() => {
                const pluginTitle = this.plugin.title;
                const current = this.checkedTypes.active[0];
                const first = this.checkedTypes.list.find((item) => item.name === current);
                this.addStep(`即将开始导出「${pluginTitle}」插件数据...`, 'detail', 1)
                this.addStep(``, 'detail', 1)
                setTimeout(() => {
        ...
        getDetail() {
            ...
            const url = '/app/xbDeveloper/admin/Index/detail'
            const data = {
                name,
            }
            this.$xbcode.useGet(url, data).then(res => {
